...
// Define the decision variables as booleans
//	x[<t, p>] = 1 if t is performed by p and 0 otherwise
dvar boolean x[couples];
...
subject to {
	...
	forall(t in tasks) {sum (<t, p> in couples) x[<t, p>] == 1;};
	
	// The maximum number of tasks assigned to a person
	max_task == max(p in persons) (sum(<t, p> in couples) x[<t, p>]);	
	
	// Linear version
	//forall(p in persons) (sum (<t, p> in couples) x[<t, p>] <= max_task);
}