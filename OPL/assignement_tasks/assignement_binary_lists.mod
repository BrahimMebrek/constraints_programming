...
//	x[t, p] = 1 if task t is performed by person p and 0 otherwise
dvar boolean x[tasks][persons];
...
subject to {
	...
	// Linear version
	//forall(p in persons) sum(t in tasks) x[t][p] <= max_task;
}
...
execute {
  ...
  for(var t in tasks) {
    for(var p in persons) {
    	if(x[t][p] == 1)
    		writeln("Task ", t, " assigned to Person ", p);
   		}    	
    }
