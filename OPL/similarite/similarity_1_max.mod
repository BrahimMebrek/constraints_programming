/* 
	...
	The problem is to partition the set of objects into k classes such that 
		we maximize the minimal intra-class similarity
*/
...
dvar int similarity_min in 0..100;
maximize similarity_min;

subject to {
  	// For each two objects we take 
  	//	the similarity if they are in the same class
  	//	or a penalty if they are not in the same class
  	// The minimum intra-class similarity is the min between all the previous results
	similarity_min == min(ordered i, j in object) ((x[i] == x[j]) * similarity[i, j] + (x[i] != x[j])*101);
	
	// Each class should contain at least 2 objects
	forall(j in class) sum(i in object) (x[i]==j) >= 2;
}